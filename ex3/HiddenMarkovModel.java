...
public class HiddenMarkovModel {
  ...
  */
  public HiddenMarkovModel (int n, double[][] transitions, Normal[][] emissions){
	...
	int i,j,k;
	int noExamples=dataList.size(); // Number of sequences in the data set
  ...
  //Sum up the number of transitions in the data class
  int[] noTransitions = new int[4];
	for(i=0;i<noStates+1;i++)
  {
    for(j=0;j<noStates+1;j++)
		  noTransitions[i] += transitionCount[i][j];
	}
  //Divide each number of transitions in each state for the total number of
  //transitions in the data class
  for(i=0;i<noStates+1;i++)
  {
    for(j=0;j<noStates+1;j++)
		  transitionProbability[i][j] = transitionCount[i][j] /
                                    (double) noTransitions[i];
	}
